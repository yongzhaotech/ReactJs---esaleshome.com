...
import { largeImageSrc, columns } from './config';
import {
...
AdInfo = {
	...
	hasPreviousColumn: () => Engine.store().NAVIGATIONCOLUMN > columns[0],
	
	hasNextColumn: () => Engine.store().NAVIGATIONCOLUMN < columns.slice(-1),

	previousColumn: () => Engine.store().NAVIGATIONCOLUMN - 1,
	
	nextColumn: () => Engine.store().NAVIGATIONCOLUMN + 1,

    currentColumn: () => Engine.store().NAVIGATIONCOLUMN,
